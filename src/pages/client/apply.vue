...
import { useLoanApplications, useChildren, useUserProfile, useNotifications } from "@/utils/useSmartLoanApi";
...
const submitApplication = async () => {
  ...
  try {
    ...
    try {
      const clientName = `${application.value.firstName} ${application.value.lastName}`;
      await createNotification({
        data: {
          type: "new_loan_application",
          client_id: currentUser.value.id,
          client_name: clientName,
          loan_amount: parseFloat(application.value.loanAmount),
          date: new Date().toISOString(),
          loan_id: createdApplication?.id,
        },
        read: false,
      });
